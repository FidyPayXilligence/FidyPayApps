...
public class HyperstreamsAPIController {

	private static final Logger LOGGER = LoggerFactory.getLogger(HyperstreamsAPIController.class);
	...
	@ApiOperation(value = "Synchronous Advanced Ckyc Download")
	@PostMapping("/synchronous/advancedCkycDownload")
	public Map<String, Object> synchronousAdvancedCkyc(@RequestHeader("Authorization") String Authorization,
			@RequestHeader(value = "Client-Id") String clientId, @RequestHeader("Client-Secret") String clientSecret,
			@RequestBody @Valid AdvancedCKycDownloadRequest advancedCKycDownloadRequest) {
		...
		try {
			...
			if (merchantInfo.getIsMerchantActive() == '1'
				...
				if (findById.isPresent()) {
					...
					return decentroServiceImpl.saveDataForSynchronousAdvancedCkyc(advancedCKycDownloadRequest,
	...
	@ApiOperation(value = "Asynchronous Advanced Ckyc Download")
	@PostMapping("/asynchronous/advancedCkycDownload")
	public Map<String, Object> asynchronousAdvancedCkyc(@RequestHeader("Authorization") String Authorization,
			@RequestHeader(value = "Client-Id") String clientId, @RequestHeader("Client-Secret") String clientSecret,
			@RequestBody @Valid AdvancedCKycDownloadRequest advancedCKycDownloadRequest) {
		...
		try {
			...
			if (merchantInfo.getIsMerchantActive() == '1'
				...
				if (findById.isPresent()) {
					...
					return decentroServiceImpl.saveDataForAsynchronousAdvancedCkyc(advancedCKycDownloadRequest,
							merchantInfo.getMerchantId(), merchants.getMerchantFloatAmount(),
							merchants.getMerchantBusinessName(), merchants.getMerchantEmail());
				}
			} else {
