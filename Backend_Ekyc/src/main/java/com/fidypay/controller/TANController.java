...
public class TANController {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(TANController.class);
	...
	@ApiOperation(value = "Vertifiy TAN Number")
	...
	public Map<String, Object> fetchPan(@RequestHeader("Authorization") String Authorization,
			...
			@Valid @RequestBody TANRequest tanRequest) {
		...
		try {
			...
			if (merchantInfo.getIsMerchantActive() == '1'
				...
				if (findById.isPresent()) {
					...
					return tanService.tanVerification(tanRequest, merchantInfo.getMerchantId(),
