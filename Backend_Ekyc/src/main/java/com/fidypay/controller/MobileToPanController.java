...
public class MobileToPanController {

	private static final Logger LOGGER = LoggerFactory.getLogger(MobileToPanController.class);
	...
	@PostMapping("/phoneTOPanDetails")
	public Map<String, Object> phoneTOPanDetails(@RequestHeader("Authorization") String Authorization,
			@RequestHeader(value = "Client-Id") String clientId, @RequestHeader("Client-Secret") String clientSecret,
			@RequestBody @Valid PanToMobileRequest panToMobileRequest) {
		...
		try {
			...
			if (merchantInfo.getIsMerchantActive() == '1'
				...
				if (findById.isPresent()) {
					...
					return decentroServiceImpl.saveDataForPhoneTOPanDetails(panToMobileRequest,
							merchantInfo.getMerchantId(), merchants.getMerchantFloatAmount(),merchants.getMerchantBusinessName(), merchants.getMerchantEmail());
				}
			} else {
