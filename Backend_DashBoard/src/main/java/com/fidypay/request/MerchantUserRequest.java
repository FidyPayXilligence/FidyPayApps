...
public class MerchantUserRequest {
	...
	@Pattern(regexp = "^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[!*-_@#$%^&+=])(?=\\S+$).*$", message = "There must be at least eight characters in the password, including at least one of each: a digit, a lowercase letter, an uppercase letter, and a special character. Whitespace is not permitted anywhere in the password.")	@NotBlank(message = "merchantPassword can not be empty")
	@Size(min = 8, max = 20, message = "merchantPassword can not be less than 8 and more than 20 words")
