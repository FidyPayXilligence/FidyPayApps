...
public class TransactionPdfExporter {
	...
	private void insertTableHeader(PdfPTable table, String text, int align, int colspan, Font font, List<String> list) {
		...
		float[] columnWidths = new float[] { 17f, 35f, 30f, 30f, 40f, 25f, 30f, 35f, 30f, 25f };
		try {
	...
	public PdfWriter generateDynamicpdf(HttpServletResponse response, List<TransactionsReportPayLoad> list2, String startDate, String endDate,
			Merchants merchants) throws ParseException {
	...
	public PdfPTable generateFirstPage(List<TransactionsReportPayLoad> activityList) {
		...
		try {
			...
			ArrayList<String> tab = new ArrayList<String>();
			tab = setTableColumns("S.No", "Transaction Date", "Service Name", "UTR", "Transaction Ref Id", "Status",
					"Amount", "Service Identifier", "Merchant Transaction Ref Id", "Merchant Order Id");
			int len = tab.size();
			...
			for (TransactionsReportPayLoad transactionsReportPayLoad : activityList) {

				insertValueIntoRow(table, "" + transactionsReportPayLoad.getsNo(), Element.ALIGN_LEFT, 1, otherfont,
						null);
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getTRXN_DATE(), Element.ALIGN_LEFT, 1,
						otherfont, null);
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getSERVICE_NAME(), Element.ALIGN_LEFT, 1,
						otherfont, null);
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getOPERATOR_REF_NO(), Element.ALIGN_LEFT, 1,
						otherfont, null);
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getTRXN_REF_ID(), Element.ALIGN_LEFT, 1,
						otherfont, null);
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getSTATUS_NAME(), Element.ALIGN_LEFT, 1,
						otherfont, null);
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getTRXN_AMOUNT(), Element.ALIGN_LEFT, 1,
						otherfont, null);

				insertValueIntoRow(table, "" + transactionsReportPayLoad.getTRXN_SERVICE_IDENTIFIER(),
						Element.ALIGN_LEFT, 1, otherfont, null);
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getMERCHANT_TRXN_REF_ID(), Element.ALIGN_LEFT,
						1, otherfont, null);
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getSP_REFERENCE_ID(), Element.ALIGN_LEFT, 1,
						otherfont, null);
			}
			return table;
		} catch (Exception e) {
