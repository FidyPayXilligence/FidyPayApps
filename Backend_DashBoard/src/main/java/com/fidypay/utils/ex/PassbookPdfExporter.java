...
public class PassbookPdfExporter {
	...
	public PdfWriter generateDynamicpdf(HttpServletResponse response, List<PassbookPayload> list2, String startDate, String endDate,
		...
		try {
			...
			doc.addTitle("Passbook Statement");
	...
	public PdfPTable generateFirstPage(List<PassbookPayload> activityList) {
		logger.info("~~~~~~~~~~PassBook Statement~~~~~~~~~");
		...
		try {
			...
			ArrayList<String> tab = new ArrayList<String>();
			tab = setTableColumns("S.No", "Transaction Date", "Merchant Transaction Ref Id", "Debit", "Credit",
					"Amount", "Service Name", "Transaction Ref Id");
			int len = tab.size();
			...
			for (PassbookPayload passbookPayload : activityList) {

				insertValueIntoRow(table, "" + passbookPayload.getsNo(), Element.ALIGN_LEFT, 1, otherfont, null);
				insertValueIntoRow(table, "" + passbookPayload.getTransactionDate(), Element.ALIGN_LEFT, 1, otherfont,
						null);
				insertValueIntoRow(table, "" + passbookPayload.getMerchantTrxnRefId(), Element.ALIGN_LEFT, 1, otherfont,
						null);
				insertValueIntoRow(table, "" + passbookPayload.getDebitAmount(), Element.ALIGN_LEFT, 1, otherfont,
						null);
				insertValueIntoRow(table, "" + passbookPayload.getCreditAmount(), Element.ALIGN_LEFT, 1, otherfont,
						null);
				insertValueIntoRow(table, "" + passbookPayload.getAmount(), Element.ALIGN_LEFT, 1, otherfont, null);
				insertValueIntoRow(table, "" + passbookPayload.getServiceName(), Element.ALIGN_LEFT, 1, otherfont,
						null);

				insertValueIntoRow(table, "" + passbookPayload.getTrxnRefId(), Element.ALIGN_LEFT, 1, otherfont, null);
			}
			return table;
		} catch (Exception e) {
