...
public class PayoutTransactionPdfExporter {
	...
	public PdfWriter generateDynamicpdf(HttpServletResponse response, List<PayoutTransactionsReportPayLoad> list2,
	...
	public PdfPTable generateFirstPage(List<PayoutTransactionsReportPayLoad> activityList) {
		...
		try {
			...
			ArrayList<String> tab = new ArrayList<String>();
			tab = setTableColumns("S. NO.", "TRANSACTION DATE", "SERVICE NAME", "UTR", "MERCHANT TRANSACTION REF ID",
					"TRXN REF ID", "TRANSACTION STATUS", "TRANSACTION AMOUNT", "CREDITOR ACCOUNT NUMBER",
					"CREDITOR IFSC", "TRANSACTION TYPE");
			int len = tab.size();
			...
			for (PayoutTransactionsReportPayLoad transactionsReportPayLoad : activityList) {
				...
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getServiceName(), Element.ALIGN_LEFT, 1,
						otherfont, null);
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getUtr(), Element.ALIGN_LEFT, 1, otherfont,
						null);
				...
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getCreditorAccountNumber(), Element.ALIGN_LEFT,
						1, otherfont, null);
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getCreditorIfsc(), Element.ALIGN_LEFT, 1,
						otherfont, null);
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getTransactionType(), Element.ALIGN_LEFT, 1,
						otherfont, null);
