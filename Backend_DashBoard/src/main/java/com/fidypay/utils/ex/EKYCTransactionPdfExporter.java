...
public class EKYCTransactionPdfExporter {
	...
	public PdfWriter generateDynamicpdf(HttpServletResponse response, List<EKYCTransactionResponse> list2,
	...
	public PdfPTable generateFirstPage(List<EKYCTransactionResponse> activityList) {
		...
		try {
			...
			tab = setTableColumns("S. NO.", "TRANSACTION DATE", "SERVICE NAME", "MERCHANT TRANSACTION REF ID",
					"TRANSACTION STATUS", "TRXN_REF_ID", "EKYC ID");
			int len = tab.size();
			...
			for (EKYCTransactionResponse transactionsReportPayLoad : activityList) {
				...
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getStatus(), Element.ALIGN_LEFT, 1, otherfont,
						null);
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getTrxnRefId(), Element.ALIGN_LEFT, 1,
						otherfont, null);
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getEkycId(), Element.ALIGN_LEFT, 1, otherfont,
						null);
			}
			return table;
		} catch (Exception e) {
