...
public class BBPSTransactionPdfExporter {

	private static final Logger logger = LoggerFactory.getLogger(BBPSTransactionPdfExporter.class);
	...
	public PdfWriter generateDynamicpdf(HttpServletResponse response, List<BBPSTransactionsReportPayLoad> list2,
	...
	public PdfPTable generateFirstPage(List<BBPSTransactionsReportPayLoad> activityList) {
		...
		try {
			...
			ArrayList<String> tab = new ArrayList<String>();
			tab = setTableColumns("S. NO.", "TRANSACTION DATE", "SERVICE NAME", "MERCHANT TRANSACTION REF ID",
					"TRANSACTION STATUS", "TRANSACTION AMOUNT", "TRXN ID", "TRANSACTION REFERENCE ID","TRXN IDENTIFIER");
			int len = tab.size();
			...
			for (BBPSTransactionsReportPayLoad transactionsReportPayLoad : activityList) {
				...
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getTransactionAmount(), Element.ALIGN_LEFT, 1,
						otherfont, null);
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getTrxnId(), Element.ALIGN_LEFT, 1, otherfont,
						null);
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getTrxnRefId(), Element.ALIGN_LEFT, 1,
						otherfont, null);
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getMobile(), Element.ALIGN_LEFT, 1,
						otherfont, null);
