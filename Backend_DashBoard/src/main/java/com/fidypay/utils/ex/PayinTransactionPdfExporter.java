...
public class PayinTransactionPdfExporter {

	private static final Logger logger = LoggerFactory.getLogger(PayinTransactionPdfExporter.class);
	...
	public PdfWriter generateDynamicpdf(HttpServletResponse response, List<PayinTransactionsReportPayLoad> list2, String startDate, String endDate,
	...
	public PdfPTable generateFirstPage(List<PayinTransactionsReportPayLoad> activityList) {
		...
		try {
			...
			ArrayList<String> tab = new ArrayList<String>();
			tab = setTableColumns("S. NO.", "TRANSACTION DATE", "SERVICE NAME", "MERCHANT TRANSACTION REF ID","TRXN REF ID",
					"TRANSACTION STATUS","TRANSACTION AMOUNT", "UTR","PAYEE VPA","PAYER VPA");
			int len = tab.size();
			...
			for (PayinTransactionsReportPayLoad transactionsReportPayLoad : activityList) {
				...
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getServiceName(), Element.ALIGN_LEFT, 1,
						otherfont, null);
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getMerchantTransactionRefId(), Element.ALIGN_LEFT, 1,
						otherfont, null);
				...
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getTransactionAmount(), Element.ALIGN_LEFT, 1,
						otherfont, null);
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getUtr(),
						Element.ALIGN_LEFT, 1, otherfont, null);
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getPayeeVpa(), Element.ALIGN_LEFT, 1,
						otherfont, null);
				insertValueIntoRow(table, "" + transactionsReportPayLoad.getPayerVpa(), Element.ALIGN_LEFT, 1,
						otherfont, null);
						}
			return table;
		} catch (Exception e) {
