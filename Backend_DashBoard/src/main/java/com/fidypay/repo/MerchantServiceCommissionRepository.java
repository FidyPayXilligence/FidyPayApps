...
public interface MerchantServiceCommissionRepository extends JpaRepository<MerchantServiceCommission, Long> {
	...
	@Query(value = "SELECT * from MERCHANT_SERVICE_COMMISSION msc where msc.MERCHANT_SERVICE_ID=?1  And msc.MERCHANT_SERVICE_COMMISSION_START=?2 And And msc.MERCHANT_SERVICE_COMMISSION_END=?3", nativeQuery = true)
	...
	@Query(value = "SELECT * from MERCHANT_SERVICE_COMMISSION msc where msc.MERCHANT_SERVICE_ID=?1", nativeQuery = true)
	List<MerchantServiceCommission> findByMerchantserviceId(long mServiceId);
